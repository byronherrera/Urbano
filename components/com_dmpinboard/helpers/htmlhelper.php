...
class PBHHtml {
	...
	function headerStuff($arg) {
		
		$document = JFactory::getDocument();
		$document->addStyleSheet('components/com_dmpinboard/css/jquery.ui.css');
		$document->addStyleSheet('components/com_dmpinboard/css/dmpinboard.css');
		if ($arg['animatelist'] == '') {
			$document->addStyleSheet('components/com_dmpinboard/css/items_anim.css');
		}
		if ($arg['load_jquery'] == 'yes') {
			$document->addScript('http://code.jquery.com/jquery-1.9.0.js');
		}
		if ($arg['load_jqueryui'] == 'yes') {
			$document->addScript('components/com_dmpinboard/js/jquery.ui.js');
		}
		$document->addScript('components/com_dmpinboard/js/jquery.masonry.min.js');
		$document->addScript('components/com_dmpinboard/js/modernizr-transitions.js');
		$document->addScript('components/com_dmpinboard/js/jquery.imagesloaded.js');
		$document->addScript('components/com_dmpinboard/js/dmjsutils.js');
		$document->addScript('components/com_dmpinboard/js/dmpinboard.js');
	}
	
	function getViewParams($source) {
		...
		$arg['infinitescroll']		= $params->get('infinitescroll','enabled');
		...
		$arg['items_offset']		= JRequest::getInt('offset',0);
		//---
		return $arg;
	}
	
	function outputView($arg) {
	...
	function getViewHtml($articles, $arg) {
	...
	function getItemsHtml($articles, $arg) {
	
		ob_start();
       	require_once(JPATH_COMPONENT.DS.'tmpl'.DS.'itemlist.php');
		$output = ob_get_contents();
		ob_end_clean();
		return $output;
	}
	
	function getPreviewHtml($preview, $arg) {
	...
	function reqOtput($result = 'OK', $type = 'HTML', $content = '', $message = '') {
