        ...
        for ( $i=0; $i < $count; $i++ )
    	{
			...
			foreach ($tmp as $link){
				...
				if ( strpos( $link, $thumb_ext ) === false ) {
		        	...
		        	$just_name = substr($image_name,strrpos($image_name,'/'));
		        	
		        	$full_url = JURI::base() . $link;
		        	$full_path = JPATH_ROOT . '/' . $link;
		        	$thumb_url_custom =  JURI::base() . $thumb_dir . '/' . $just_name . $thumb_ext . $extension;
		        	$thumb_path_custom = JPATH_ROOT. '/' . $thumb_dir . '/' . $just_name . $thumb_ext . $extension;
		        	$thumb_url = JURI::base() . $image_name . $thumb_ext . $extension;
		        	$thumb_path = JPATH_ROOT . '/' . $image_name . $thumb_ext . $extension;
